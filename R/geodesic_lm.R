## geodesic.lm.control is called from nlsLM, used to find convergence parameters
## for the nlsLM algorithm.
geodesic.lm.control <- function(ftol  = sqrt(.Machine$double.eps),
...
## geodesic.lm is called from nlsLM and calls the optimisation routines through
## the external call to nls_lm.
geodesic.lm <- function(par, lower=NULL, upper=NULL, fn, jac = NULL, acc = NULL, 
                   callback = NULL, control = geodesic.lm.control(), ...){
    ...
    class(out)   <- "geodesic.lm"
    out
}

print.geodesic.lm <- function(x, digits = max(3, getOption("digits") - 3), ...){
    
    cat("Nonlinear regression via the Improved Levenberg-Marquardt algorithm\n")
...
deviance.geodesic.lm    <- function(object, ...) object$deviance
coef.geodesic.lm        <- function(object, ...) unlist(object$par)
residuals.geodesic.lm   <- function(object, ...) object$fvec
df.residual.geodesic.lm <- function(object, ...){
    length(resid(object)) - length(coef(object))
}

summary.geodesic.lm <- function (object, ...){
    ...
    ans <- list(residuals = object$fvec, sigma = sqrt(object$deviance/rdf),
                ...
                coefficients = param)
    class(ans) <- "summary.geodesic.lm"
    ans
}

print.summary.geodesic.lm <- function (x, digits = max(3, getOption("digits") - 3), ...){
...
vcov.geodesic.lm <- function(object,...){
    object$deviance/(length(object$fvec)-length(object$par))*solve(object$hessian)
}

confint.geodesic.lm <- function(object, parm, level = 0.95, ...){
    cc <- coef(object)
